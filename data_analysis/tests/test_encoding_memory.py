...
FIGURES = [
    (
        "relational_split",
        "Action vs Spatial relational recall — encoding scanpath",
        [
    ...
    (
        "relational_combined",
        "Relational recall (combined) — encoding scanpath",
        [
            (
    ...
    (
        "objects_combined",
        "Object recall (identity + features) — encoding scanpath",
        [
...
def _print_summary(records: list[dict], fig_title: str) -> None:
    print(f"\n  {fig_title}")
    print("  " + "-" * 68)
...
def main():
    ...
    for suffix, title, dv_specs in FIGURES:
        out_path = Path(args.output) / f"encoding_memory_{suffix}.png"
        if not args.no_plot:
            records = _make_figure(df, title, dv_specs, out_path, pred_resids)
        else:
            records = _compute_only(df, dv_specs, pred_resids)
