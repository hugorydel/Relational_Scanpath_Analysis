...
Author: T.R. Hayes
Version: 1.0.0
Description: Computes Masked Autoencoders (MAE) embeddings for input images.

He, K., et al. (2022). Masked Autoencoders Are Scalable Vision Learners. 
In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern 
Recognition (CVPR), pp. 16000-16009.

Pretrained ViT-B/16 weights downloaded from:
  https://dl.fbaipublicfiles.com/mae/pretrain/mae_pretrain_vit_base.pth

See MAE page for more details:
  https://github.com/facebookresearch/mae

Changelog:
- 1.0.0 (2024-12-01): TRH Wrote it
"""
...
class MAEFeatureExtractor(torch.nn.Module):
    """MAE embedding extraction module."""
...
def get_transform():
    """Get preprocessing transform for MAE."""
...
def extract_embeddings(
    ...
    """Extract embeddings for MAE model.
    
    This function processes batches of images through the MAE model to extract
    image embeddings, while also tracking the corresponding image paths.
    
    Args:
        model (torch.nn.Module): The MAE feature extractor model
    ...
    Returns:
        Tuple[np.ndarray, List[str]]: A tuple containing:
            - np.ndarray: Array of shape (n_samples, embedding_dim) with image embeddings
...
def get_embeddings(
    ...
    """Extract MAE embeddings from images.
    
    This function:
    1. Attempts to load existing embeddings if available and not forced to recompute
    2. Otherwise loads the MAE model with pre-trained weights
    ...
    Args:
        ...
        save_names (Optional[List[str]], optional): List of [embeddings_filename, paths_filename].
            Defaults to ['mae_embeddings.npy', 'mae_paths.npy'].
    ...
    Returns:
        Tuple[np.ndarray, Dict[str, int]]: A tuple containing:
            - np.ndarray: Array of shape (n_samples, embedding_dim) with MAE embeddings
    ...
    if save_names is None:
        save_names = ['mae_embeddings.npz', 'mae_paths.npz']
    ...
    base_model.heads = torch.nn.Identity()
    model = MAEFeatureExtractor(base_model).to(device)
