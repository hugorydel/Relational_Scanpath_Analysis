...
IMAGES_DIR = config.DATA_METADATA_IMAGES_DIR
...
5. PRESERVE FIELDS
   ...
   - source_phrases for split or corrected nodes should contain only the phrases that actually map to that specific concept.
...
def load_processed_stims() -> set:
    """Return set of StimIDs already in edit_results.jsonl."""
    processed = set()
        ...
        for line in f:
            ...
            if line:
                try:
                    ...
                    continue
    logger.info(f"Found {len(processed)} already-edited stim(s) in edit_results.jsonl.")
    return processed
...
async def run(args) -> None:
    ...
    if not args.force:
        ...
        stims = [s for s in stims if s not in processed]
        if not stims:
            logger.info("All stims already edited. Use --force to rerun.")
            return
