...
def build_graph_index(metadata_file: Optional[Path] = None) -> dict:
    ...
    for stim_id, entry in metadata.items():
        ...
        for rel in entry.get("relations", []):
            ...
            edges_all.add(frozenset({subj, obj}))
    ...
    logger.info(
        ...
        f"Unique undirected core edges (inter+spatial+func): "
        f"{sum(len(v) for v in all_edges.values())}."
    )
    return {"all": all_edges}
