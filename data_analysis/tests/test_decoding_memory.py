"""
tests/test_decoding_memory.py
==============================
Standalone test: decoding-time scanpath quality → episodic memory.

Mirror of test_encoding_memory.py but for the decoding (free-recall) phase.

Predictors (decoding phase)
---------------------------
  svg_z_inter_dec  — interactional-edges SVG z-score during free recall
  svg_z_all_dec    — all-edges SVG z-score during free recall
...
Note: decoding trials require a memory score to merge, so n will be smaller
than the encoding module (~99 trials vs ~109).

Output
------
  output/analysis/decoding_memory.png
    2×2 grid of partial regression scatters (rows=DV, cols=predictor)
    + 1 summary bar chart of OLS β coefficients

Usage
-----
    python tests/test_decoding_memory.py
    python tests/test_decoding_memory.py --features path/to/trial_features_all.csv
    python tests/test_decoding_memory.py --scores   path/to/memory_scores.csv
    python tests/test_decoding_memory.py --output   path/to/output/analysis
    python tests/test_decoding_memory.py --no-plot
"""
...
def _load(features_path: Path, scores_path: Path) -> pd.DataFrame:
    """
    Join decoding trial features with memory scores on SubjectID × StimID.
    Only trials with both a decoding SVG score and a memory score are kept.
    """
    ...
    # Inner join: only trials with both a decoding SVG and a memory score
    merged = dec.merge(scores, on=["SubjectID", "StimID"], how="inner")

    # Exclude low-n decoding trials (too few AOI fixations for a reliable SVG)
...
def _print_summary(coef_records: list) -> None:
    print("\n" + "=" * 72)
    print("DECODING SCANPATH → MEMORY  (partial regression, covariates removed)")
    print("=" * 72)
...
def _plot(df: pd.DataFrame, coef_records: list, output_path: Path) -> None:
    ...
    ax_summary.set_title(
        "Summary: decoding predictor → memory (covariate-adjusted)",
        fontsize=9,
        fontweight="bold",
        pad=6,
    )
    ...
    fig.suptitle(
        "Decoding scanpath → episodic memory\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Decoding scanpath → memory: partial regression plots."
    )
    ...
    print("\n" + "=" * 60)
    print("Decoding scanpath → episodic memory")
    print("=" * 60)
    ...
    if not args.no_plot:
        _plot(df, coef_records, Path(args.output) / "decoding_memory.png")
